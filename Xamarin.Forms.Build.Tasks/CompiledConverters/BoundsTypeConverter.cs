	...
	class BoundsTypeConverter : ICompiledTypeConverter
	{
		public IEnumerable<Instruction> ConvertFromString(string value, ILContext context, BaseNode node)
		{
			...
			if (string.IsNullOrEmpty(value))
				throw new XamlParseException($"Cannot convert \"{value}\" into {typeof(Rectangle)}", node, errorCode: "CSXF1207");
			...
			if (xywh.Length != 2 && xywh.Length != 4)
				throw new XamlParseException($"Cannot convert \"{value}\" into {typeof(Rectangle)}", node, errorCode: "CSXF1208");
			...
			if (!hasX || !hasY || !hasW || !hasH)
				throw new XamlParseException($"Cannot convert \"{value}\" into {typeof(Rectangle)}", node, errorCode: "CSXF1209");
